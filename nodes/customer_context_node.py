...
def _run_attack_reports(customers:list[dict],network:str)->list[dict]:
    if not customers:
        return []
    attack_results=[]
    for customer in customers:
        ...
        try:
            result=find_attack_context(customer_id,customer_name,network)
...
def _format_customer_output(customer_result:list[dict],customer_error:str|None)->dict:
    if customer_error:
...
def customer_context_node(state:dict)->dict:
